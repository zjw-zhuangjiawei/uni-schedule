		...
		fn rebalance(mut self: Box<Self>) -> Box<Node> {
			...
			if bf > 1 {
				...
				if Node::height(&left_ref.right) > Node::height(&left_ref.left) {
					...
					let left =
						unsafe { self.left.take().map(|n| n.rotate_left()).unwrap_unchecked() };
					self.left = Some(left);
				}
			...
			D: serde::Deserializer<'de>,
		{
			...
			impl<'de> Visitor<'de> for LapperVisitor {
					...
					V: MapAccess<'de>,
				{
					...
					let intervals =
						intervals.ok_or_else(|| de::Error::missing_field("intervals"))?;
	...
	mod tests {
		...
		mod interval_tests {
			...
			fn overlap_with_minimal_durations() {
				...
				assert!(tiny_interval
					.overlap(start - Duration::seconds(1), start + Duration::seconds(1)));
			}
		...
		mod lapper_core_tests {
			...
			fn single_interval_operations() {
				...
				assert!(lapper
					.has_overlap(start + Duration::minutes(30), start + Duration::minutes(45)));

				// Should find overlap with overlapping query
				assert!(
					lapper.has_overlap(start + Duration::minutes(30), start + Duration::hours(2))
				);
		...
		mod data_structure_tests {
			...
			fn avl_tree_rebalances_under_skewed_insertions() {
				...
				assert!(
					lapper.has_overlap(start + Duration::minutes(30), start + Duration::hours(1))
				);
				assert!(lapper.has_overlap(
		...
		mod serialization_tests {
			...
			fn serialization_preserves_functionality() {
				...
				let original_overlaps: Vec<_> =
					original.find(query_start, query_stop).cloned().collect();
		...
		mod edge_case_tests {
			...
			fn query_with_zero_length_range() {
				...
				assert!(!lapper
					.has_overlap(start + Duration::minutes(30), start + Duration::minutes(30)));
			...
			fn invalid_query_range_with_start_greater_than_stop() {
				...
				assert!(!lapper
					.has_overlap(start + Duration::minutes(45), start + Duration::minutes(30)));
			}
		...
		mod partition_point_tests {
			...
			fn lower_bound_behavior() {
				...
				let lb_after =
					intervals.partition_point(|iv| iv.start < start + Duration::hours(6));
